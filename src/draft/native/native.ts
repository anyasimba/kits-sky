...
export class Native extends CircleOfLife {
    dead: boolean;
    [__.$$native]: unknown
    ...
    constructor() {
        ...
        this[__.$$native] = this[__.$$nativeConstructor]()
        const key = (this[__.$$native] as any).pointer()
        nativeWraps[key] = this
    }
}
...
export function native_class(name, ...mixins) {
    return function (constructor) {
        ...
        {
            const get = native[`${name}_get_dead`]
            const set = native[`${name}_set_dead`]
...
function apply(name, prototype, props, methods) {
